...
const GRADIENT_MAP: Record<string, string> = {
  ...
  "gradient-amber": "from-amber-500 to-amber-600",
...
function StatCard({ stat }: { stat: typeof DASHBOARD_STATS[0] }) {
  ...
  return (
    <div className={cn("rounded-3xl p-4 text-white min-w-[140px] flex-shrink-0 flex flex-col gap-3", `bg-gradient-to-br ${GRADIENT_MAP[stat.gradient] || "from-blue-500 to-blue-600"}`)}>
      <div className="flex items-center justify-between">
        <div className="w-9 h-9 rounded-xl bg-white/20 flex items-center justify-center">
          <Icon size={18} />
        </div>
        {stat.change && (
          <span className="flex items-center gap-0.5 text-[11px] font-semibold bg-white/20 px-2 py-0.5 rounded-full">
            {stat.changeType === "up" ? <TrendingUp size={10} /> : stat.changeType === "down" ? <TrendingDown size={10} /> : null}
            {stat.change}
          </span>
        )}
      </div>
      <div>
        ...
        <p className="text-white/80 text-[13px] mt-0.5">{stat.label}</p>
      </div>
    </div>
  );
}
...
function ScheduleCard({ item }: { item: typeof TODAY_SCHEDULE[0] }) {
  const statusConfig = {
    completed: { color: "text-text-secondary", dot: "bg-slate-300", label: "Done" },
    live: { color: "text-emerald-600", dot: "bg-emerald-500 animate-pulse", label: "Live" },
    upcoming: { color: "text-text-primary", dot: "bg-primary", label: "Next" },
  };
  const s = statusConfig[item.status];

  return (
    <div className={cn("card card-hover flex items-center gap-4", item.status === "live" && "ring-2 ring-emerald-400")}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="font-semibold text-body text-text-primary truncate">{item.className}</p>
          {item.status === "live" && (
            <Badge variant="green" dot>Live</Badge>
          )}
        </div>
        <p className="text-caption text-text-secondary">{item.time} · {item.room}</p>
      </div>
      {item.status !== "completed" && (
        <div className="flex gap-2">
          {item.status === "live" ? (
            <Button size="sm" variant="success" icon={<Play size={14} />}>Join</Button>
          ) : (
            <Button size="sm" variant="secondary" icon={<Play size={14} />}>Start</Button>
          )}
        </div>
      )}
      {item.status === "completed" && (
...
function AlertCard({ alert }: { alert: typeof STUDENT_ALERTS[0] }) {
  ...
  return (
    <div className={cn("card card-hover p-4", c.bg)}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <p className="font-semibold text-body text-text-primary truncate">{alert.studentName}</p>
            <span className={cn("w-2 h-2 rounded-full flex-shrink-0", c.dot)} />
            <Badge variant={alert.severity === "critical" ? "red" : alert.severity === "medium" ? "amber" : "blue"}>
              {alert.severity}
            </Badge>
          </div>
          <p className="text-caption text-text-secondary mb-2">{alert.message}</p>
          <p className="text-[12px] text-text-secondary bg-white/60 px-2.5 py-1 rounded-xl inline-block">
...
function AIRecommendationCard({ rec }: { rec: typeof AI_RECOMMENDATIONS[0] }) {
  const icons = { remedial: "🧠", quiz: "📝", revision: "🔄", grouping: "👥" };
  return (
    <div className="card card-hover flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-body text-text-primary">{rec.title}</p>
        <p className="text-caption text-text-secondary mt-0.5 line-clamp-2">{rec.description}</p>
      </div>
      <Button size="sm" variant="ai">{rec.actionLabel}</Button>
    </div>
  );
}
    ...
    <Link href={`/classes/${cls.id}`}>
      <div className="card card-hover">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="font-semibold text-body text-text-primary">{cls.name}</p>
            <p className="text-caption text-text-secondary">{cls.studentCount} students</p>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-caption">
            ...
            <span className="font-semibold text-text-primary">{cls.attendanceToday}%</span>
          </div>
          <ProgressBar value={cls.attendanceToday} variant="blue" size="sm" />
          <div className="flex items-center justify-between text-caption">
            <span className="text-text-secondary">Homework</span>
            <span className="font-semibold text-text-primary">{cls.homeworkStatus}%</span>
          </div>
          <ProgressBar value={cls.homeworkStatus} variant="green" size="sm" />
          <div className="flex items-center justify-between text-caption">
            <span className="text-text-secondary">Engagement</span>
            <span className="font-semibold text-text-primary">{cls.engagementScore}%</span>
          </div>
          <ProgressBar value={cls.engagementScore} variant="purple" size="sm" />
        </div>
        {cls.weakStudentsCount > 0 && (
          <div className="mt-3 flex items-center gap-1.5 text-caption">
            <AlertTriangle size={12} className="text-red-500" />
            <span className="text-red-600 font-medium">{cls.weakStudentsCount} students need attention</span>
          </div>
        )}
      </div>
    </Link>
    ...
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="md:hidden">
        <h1 className="text-app-title font-bold text-text-primary">
          {greeting}, <span className="text-gradient-blue">Ananya Ma&apos;am</span> 👋
        </h1>
        <p className="text-text-secondary mt-1">
          Saturday, 16 May 2026 · Tarkon International School
        </p>
      </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href="/analytics" className="text-caption text-primary font-medium flex items-center gap-1 hover:underline">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/classes" className="text-caption text-primary font-medium flex items-center gap-1 hover:underline">
                All Classes <ArrowRight size={12} />
              </Link>
            </div>
            <div className="space-y-2">
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              <h2 className="section-title">🚨 Student Alerts</h2>
              <Badge variant="red" dot>{STUDENT_ALERTS.filter(a => a.severity === "critical").length} Critical</Badge>
            </div>
            <div className="space-y-3">
              {STUDENT_ALERTS.map((alert) => (
                <AlertCard key={alert.id} alert={alert} />
          ...
          {/* Pending Reviews */}
          <section>
            <h2 className="section-title mb-3">📋 Pending Reviews</h2>
            <div className="space-y-2">
              {[
                { label: "Fractions Worksheet", sub: "28/38 submitted · Grade 5A", type: "Homework", action: "Review" },
                { label: "English Grammar Test", sub: "8 ungraded · Grade 7A", type: "Exam", action: "Grade" },
                { label: "Parent Meeting Request", sub: "Rahul's parent · Urgent", type: "Parent", action: "Respond" },
              ].map((item) => (
                <div key={item.label} className="card card-hover flex items-center gap-4">
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-body text-text-primary">{item.label}</p>
                    <p className="text-caption text-text-secondary">{item.sub}</p>
                  </div>
                  <Badge variant={item.type === "Parent" ? "red" : item.type === "Exam" ? "amber" : "blue"}>
                    {item.type}
                  </Badge>
                  <Button size="sm" variant="secondary">{item.action}</Button>
                </div>
              ))}
            </div>
          </section>
        ...
        <div className="space-y-6">
          {/* Class Health */}
          <section>
            <h2 className="section-title mb-3">🏫 Class Health</h2>
            <div className="space-y-3">
              {CLASSES.map((cls) => (
                <ClassHealthCard key={cls.id} cls={cls} />
              ))}
